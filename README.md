...
**Source file:** `financial_loan_data_excel.xlsx` (cleaned to `financial_loan_cleaned.csv` in Python)
...
- Exported the cleaned dataset for use in Power BI / SQL
...
│
├── data/
│   ├── raw/financial_loan_data_excel.xlsx
│   └── cleaned/financial_loan_cleaned.csv
│
├── python/
...
1. Clone this repo and place the raw dataset in `data/raw/`.
2. Open `python/finance_project_python.ipynb` and run all cells to clean the data and generate KPIs — this produces `financial_loan_cleaned.csv`.
3. (Optional) Load the cleaned CSV into a SQL database and run `sql/business_queries.sql` for segment-level querying.
4. Open `powerbi/financial_loan_dashboard.pbix` in Power BI Desktop, point it to the cleaned CSV, and hit Refresh to rebuild the dashboard.

---

## 🙋 About This Project

Built as a data analyst portfolio project to demonstrate the full analytics workflow — from raw data cleaning and KPI derivation in Python, through business querying in SQL, to a polished, interactive Power BI dashboard.
